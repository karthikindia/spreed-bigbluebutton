...
const getFileConversation = async function({ fileId }, options) {
	const response = await axios.get(generateOcsUrl('apps/talk_bbb/api/v1', 2) + `file/${fileId}`)
	return response
}
...
const getPublicShareConversationData = async function(shareToken) {
	const response = await axios.get(generateOcsUrl('apps/talk_bbb/api/v1', 2) + `publicshare/${shareToken}`)
